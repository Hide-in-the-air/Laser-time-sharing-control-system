...
void bsp_InitTimer(void)
{
    ...
    */
	...
	//最小中断周期为1ms
	bsp_InitHardTimer(MINI_TIMER_USPRD);
	
	//bsp_StartHardTimer(2, 1000, timer_stick);
}
...
*	形    参: _usPeriod: us为单位的个数 1000 = 1ms
*	返 回 值: 无
*********************************************************************************************************
*/
void bsp_InitHardTimer(uint32_t _usPeriod)
{
  ...
  */
	TimHandle.Instance = TIMx;			//已经是TIM_HARD = TIM2了
...
{	
	...
	//if ((itstatus != (uint16_t)RESET) && (itenable != (uint16_t)RESET))
	{
		...
		//
		//禁能CC2中断 此处不适用，保持中断即可
		//TIMx->DIER &= (uint16_t)~TIM_IT_CC2;		
