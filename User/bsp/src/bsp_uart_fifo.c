...
extern void rev_from_pc(uint8_t _byte);
extern void rev_from_nimma900(uint8_t _byte);
extern void rev_from_nimma600(uint8_t _byte);
...
static void UartVarInit(void)
{
	...
	g_tUart1.ReciveNew = rev_from_nimma900;						/* 接收到新数据后的回调函数 */
	...
	g_tUart2.ReciveNew = rev_from_pc;						/* 接收到新数据后的回调函数 */
	...
	//dbg@0521
	g_tUart4.ReciveNew = rev_from_pc;						/* 接收到新数据后的回调函数 */
	...
	g_tUart6.ReciveNew = rev_from_nimma600;						/* 接收到新数据后的回调函数 */
